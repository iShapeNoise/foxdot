...
class NumberKey(object):
    """
    An object that acts like a number but may have dependencies when returing
    its value. These are used when returing Player object attribute values
    accessed via `getattr` e.g. `p1.dur` or `getattr(p1, "dur")`. The parent
    attribute is the Player that contains the key or the referenced key in the
    case that the value has been maniupulated. e.g. if `p1` is using
    `p2.degree` then `p1.degree.parent == p1` and
    ...
    def index(self, sequence):
        """
        Returns a Player Key that returns the element from sequence indexed
        using int(self)
        """
    ...
    def semitones(self):
        """
        Converts the current value into the semitone value using the parent's
        scale
        """
        new = self.child(0)
    ...
    def simple_map(self, mapping):
        """
        Creates a new Player key that maps the values in the dictionary
        (mapping) to new values. Example use case:
    ...
    def versus(self, rule=lambda x, y: x > y):
        ...
        """
        # 1. Sets this source player key amplify to be "off" when the rule
        # is satisfied
...
class PlayerKey(NumberKey):
    ...
    def __init__(self, value, function=None, player=None, attr=None):
        ...
        # if self.parent is not None else asStream([]) #
    ...
    def update(self, value, time):
        """
        Updates the contents of the PlayerKey *if* the time value is different
        to self.last_updated. If they are the same, the the contents become a
        PGroup of the two values
        """
...
class Accompany:
    ...
    def __call__(self, playerkey):
        ...
        # Only change value if the player key has changed
        # - maybe set a frequency?
