...
# Sample Player, Loop Player
with SampleSynthDef("play1") as play:
    play.osc = PlayBuf.ar(1,
                          play.buf,
...
# Sample Player
if os.listdir(SYNTHDEF_DIR) != []:
    # Read synths from file
    synths_list = os.listdir(SYNTHDEF_DIR)
    synths_list.sort()
    for synth in synths_list:
        if (synth == "play1.scd") or (synth == "play2.scd") or (synth == "loop.scd"):
            pass
        else:
            synth_name = os.path.splitext(synth)
            synth_name = synth_name[0]
            globals()[synth_name] = FileSynthDef(synth_name)
            globals()[synth_name].add()
else:
    # Synth Players
    ...
    with SynthDef("dab") as synth:
         ...
         b = VarSaw.ar(synth.freq / 4,
                       mul=synth.amp,
                       width=Env.perc(synth.sus / 20,
                                      synth.sus / 4,
                                      0.5,
                                      -5,
                                      doneAction=0))
    ...
    with SynthDef("klank") as klank:
        ...
        klank.osc = Klank.ar([[1, 2, 3, 4], [1, 1, 1, 1], [2, 2, 2, 2]],
                             ClipNoise.ar(0.0005).dup, klank.freq)
        if SC3_PLUGINS:
    ...
    with SynthDef("glass") as glass:
        ...
        glass.osc = Klank.ar([[2, 4, 9, 16], [1, 1, 1, 1], [2, 2, 2, 2]],
                             PinkNoise.ar(0.0005).dup * SinOsc.ar(glass.freq / 4, add=1, mul=0.5), glass.freq)
