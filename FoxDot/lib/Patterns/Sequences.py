...
class __pattern__(object):
    ...
    Ranges can be created using slicing, e.g. `P[1:6:2]` will generate the
    range 1 to 6 in steps of 2, thus creating the Pattern `[1, 3, 5]`. Slices
    can be combined with other values in a Pattern such that `P[0,2,1:10]`
    will return the Pattern `P[0, 2, 1, 2, 3, 4, 5, 6, 7, 8, 9]`
    '''
    ...
    def __xor__(self, other):
        """
        P^(x1, x2,..., dur) -  Returns a PGroup that delays each value by
        dur * n
        """
...
def PZip(pat1, pat2, *patN):
    ''' Creates a Pattern that 'zips' together multiple patterns.
    `PZip([0,1,2], [3,4])` will create the Pattern
    `P[(0, 3), (1, 4), (2, 3), (0, 4), (1, 3), (2, 4)]`
    '''
    l, p = [], []
...
def PZip2(pat1, pat2, rule=lambda a, b: True):
    '''
    Like `PZip` but only uses two Patterns. Zips together values if they
    satisfy the rule.
    '''
...
def PStutter(x, n=2):
    """
    PStutter(seq, n) -> Creates a pattern such that each item in the array is
    repeated n times (n can be a pattern)
    """
...
def PTri(start, stop=None, step=1):
    """
    Returns a Pattern equivalent to ``Pattern(range(start, stop, step))`` with
    its reversed form appended.
    """
    if stop is None:
        start, stop = 0, start
...
def PEuclid(n, k):
    '''
    Returns the Euclidean rhythm which spreads 'n' pulses over 'k' steps as
    evenly as possible.
...
def PEuclid2(n, k, lo, hi):
    '''
    Same as PEuclid except it returns an array filled with 'lo' value instead
    of 0 and 'hi' value instead of 1. Can be used to generate characters
    patterns used to play sample like play(PEuclid2(3,8,'-','X')) will be
    equivalent to
        play(P['X', '-', '-', 'X', '-', '-', 'X', '-'])
...
def PDur(n, k, start=0, dur=0.25):
    """
    Returns the *actual* durations based on Euclidean rhythms (see PEuclid)
    where dur is the length of each step.
        ::
            >>> PDur(3, 8)
            P[0.75, 0.75, 0.5]
            >>> PDur(5, 16)
    ...
    """
    # If we have more pulses then steps, double the steps and decrease
    # the duration
    while n > k:
        k = k * 2
        dur = dur / 2
...
# forces it into a stream instead of Group
@loop_pattern_func
def PDelay(*args):
...
def PRhythm(durations):
    """
    Converts all tuples/PGroups into delays calculated using the
    PDur algorithm.
    e.g.
