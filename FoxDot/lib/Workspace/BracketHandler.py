...
class BracketHandler:
    ...
    def handle(self, event=None, insert=INSERT):
        ...
        if event.char in self.left_brackets:
            ...
            try:
                ...
                pass
            # B. If next character is a whitespace or a right bracket add
            # a closing bracket as well.
            if next_char in whitespace + self.right_brackets_only:
                ...
                if event.char in ("'", '"'):
                    if event.char == next_char:
                        ...
                        # Only add a closing quote mark if there are already
                        # an even no. of quotes in the line
        ...
        # 2. Type right bracket
            ...
            # Add bracket
            self.text.insert("{}.{}".format(line, column), event.char)
    ...
    def delete(self, insert=INSERT):
        """
        Deletes enclosed set of brackets if no contents and returns True,
    ...
    def find_starting_bracket(self, line, column, bracket_style, offset=0):
        """
        Finds the opening bracket to the closing bracket at line, column
        co-ords. Returns None if not found. Only goes as far as the first line
        visible on screen
        """
