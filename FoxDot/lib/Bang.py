"""
This is an experimental module that contains the `Bang` class; events that are
triggered in time with `Player` rhythms to give visual aid to which players are
    ...
    def __init__(self, player, kwargs):
        ...
        if self.func:
            ...
            if a <= player.line_number <= b:
                ...
                # clock.latency
                message_time = player.get_timestamp(player.queue_block.beat)
                ...
                def bang():
                    ...
                    while time() < message_time:
                        sleep(0.001)
                    self.widget.addTask(target=self.widget.text.tag_add,
                                        args=(self.id,
                                              start,
                                              end))
                    self.widget.addTask(target=self.widget.text.tag_config,
                                        args=(self.id,),
                                        kwargs=kwargs)
