...
class Repeatable(object):
    ...
    def update_pattern_methods(self, attr):
        """
        Update the 'current' version of a pattern based on its root and methods
        stored
        """
        if attr not in self.previous_patterns:
            self.previous_patterns[attr] = MethodList(self.attr[attr])
    ...
    def every(self, occurence, cmd, *args, **kwargs):
        """ Every n beats, call a method (defined as a string) on the
            ...
            ::
                ...
                # If the method is not valid but *is* a valid Pattern method,
                # that is called and reverted
        ...
        if cmd in self.repeat_events:
            ...
            if self.is_pattern_method(method_name, attr):
                # if n is even, it means the method is active
                # ToDo -- put this in a class mate
                n, acc = call.count()
                if n % 2 == 1:
...
class MethodCall:
    ...
    def update(self, n, cycle=None, args=(), kwargs={}):
        """
        Updates the values of the MethodCall. Re-adjusts the index if cycle has
        been changed
        """
    ...
    def count(self):
        """
        Counts the number of times this method would have been called between
        clock start and now
        """
        n = 0
        acc = 0
        ...
        # Get durations
        # if self.cycle is None else asStream(self.cycle)
        durations = self.when
    ...
    def __call__(self, *args, **kwargs):
        """
        Proxy for parent object __call__, calls the enclosed method and
        schedules it in the future.
        """
